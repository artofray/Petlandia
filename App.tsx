...
import { Grid, TileData, BuildingType, CityStats, AIGoal, NewsItem, PetAvatar } from './types';
import { GRID_SIZE, BUILDINGS, TICK_RATE_MS, INITIAL_MONEY } from './constants';
import IsoMap from './components/IsoMap';
import UIOverlay from './components/UIOverlay';
import StartScreen from './components/StartScreen';
import PetAvatarEditor from './components/PetAvatarEditor';
import { generateCityGoal, generateNewsEvent, generatePetThought } from './services/geminiService';
...
function App() {
  ...
  const [stats, setStats] = useState<CityStats>({ money: INITIAL_MONEY, population: 0, day: 1, level: 1 });
  const [selectedTool, setSelectedTool] = useState<BuildingType>(BuildingType.Road);
  
  // --- Avatar State ---
  const [avatar, setAvatar] = useState<PetAvatar>({ name: 'Mayor Fluff', species: 'Dog', color: '#facc15' });
  const [isEditingAvatar, setIsEditingAvatar] = useState(false);
  const [petThought, setPetThought] = useState<string>("Welcome to Petlandia!");
  ...
  const updatePetThought = useCallback(async () => {
    const s = statsRef.current;
    
    if (s.level >= 50 && aiEnabledRef.current) {
      // Translate
      const thought = await generatePetThought(s, avatarRef.current);
      setPetThought(thought);
    } else {
      // Gibberish / Sound based on species
      const species = avatarRef.current.species;
      const sounds: Record<string, string[]> = {
        'Dog': ['Woof!', 'Bork bork.', 'Sniffing...', '*Wag*'],
        'Cat': ['Meow.', 'Purrrr...', 'Hiss!', 'Nap time.'],
        'Bunny': ['*Hop*', '*Nibble*', 'Twitch.', '...'],
        'Dragon': ['Rawr!', '*Smoke*', 'Grrr...', 'Gold!'],
        'Ghost': ['Booo...', 'Whoosh.', '*Float*', '...'],
        'Rock': ['...', '...', '...!', '(Heavy silence)'],
      };
      const opts = sounds[species] || ['...'];
      setPetThought(opts[Math.floor(Math.random() * opts.length)]);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    addNewsItem({ id: Date.now().toString(), text: "Welcome to Petlandia! Dig in.", type: 'positive' });
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      ...
      setStats(prev => {
        ...
        const newLevel = Math.max(1, Math.floor(newPop / 10) + 1);
      ...
      // 4. Trigger news & thoughts
      fetchNews();
      
      // Update thought less frequently (every 5 ticks approx, handled by random chance inside or counter)
      if (Math.random() > 0.7) updatePetThought();

    }, TICK_RATE_MS);

    return () => clearInterval(intervalId);
  }, [fetchNews, gameStarted, updatePetThought]);


  // --- Interaction Logic ---

  const handleTileClick = useCallback((x: number, y: number) => {
    if (!gameStarted || isEditingAvatar) return; // Prevent clicking through
  ...
  }, [selectedTool, addNewsItem, gameStarted, isEditingAvatar]);
    ...
    <div className="relative w-screen h-screen overflow-hidden selection:bg-transparent selection:text-transparent bg-sky-900">
      ...
      {isEditingAvatar && (
        <PetAvatarEditor 
          ...
          onSave={(newAvatar) => { setAvatar(newAvatar); setIsEditingAvatar(false); }}
